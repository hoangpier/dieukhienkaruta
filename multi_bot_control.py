...
def create_bot(token, is_main=False, is_main_2=False):
    ...
    # <<< PHẦN CODE ĐÃ SỬA LỖI >>>
    # Hợp nhất logic on_message cho cả hai bot chính vào một chỗ.
    # Chỉ những bot được đánh dấu is_main hoặc is_main_2 mới có trình xử lý này.
    if is_main or is_main_2:
        @bot.gateway.command
        def on_message(resp):
            ...
            # Chỉ xử lý tin nhắn từ Karuta trong kênh chính
            if author != karuta_id or channel != main_channel_id:
                return
            
            content = msg.get("content", "")
            mentions = msg.get("mentions", [])
            
            # Kiểm tra xem đây có phải là tin nhắn drop không
            if "is dropping" in content or mentions:
                return

            # Xác định xem bot nào đang chạy để áp dụng đúng cài đặt
            is_bot_1 = (bot.token == main_token)
            
            current_auto_grab_enabled = auto_grab_enabled if is_bot_1 else auto_grab_enabled_2
            current_heart_threshold = heart_threshold if is_bot_1 else heart_threshold_2
            bot_name_log = "[Bot 1]" if is_bot_1 else "[Bot 2]"

            if not current_auto_grab_enabled:
                return

            print(f"\n{bot_name_log} Phát hiện tự drop! Đọc tin nhắn Karibbit...\n")
            last_drop_msg_id = msg["id"]

            def read_karibbit():
                time.sleep(0.5)
                messages = bot.getMessages(main_channel_id, num=5).json()
                for karibbit_msg in messages:
                    author_id = karibbit_msg.get("author", {}).get("id")
                    if author_id == karibbit_id and "embeds" in karibbit_msg and len(karibbit_msg["embeds"]) > 0:
                        desc = karibbit_msg["embeds"][0].get("description", "")
                        print(f"\n{bot_name_log} ===== Tin nhắn Karibbit đọc được =====\n{desc}\n{bot_name_log} ===== Kết thúc tin nhắn =====\n")
                        ...
                        for i, line in enumerate(lines[:3]):
                            ...
                            if len(matches) >= 2 and matches[1].isdigit():
                                ...
                                print(f"{bot_name_log} Dòng {i+1} số tim: {num}")
                            else:
                                heart_numbers.append(0)
                                print(f"{bot_name_log} Dòng {i+1} không tìm thấy số tim, mặc định 0")

                        if sum(heart_numbers) == 0:
                            print(f"{bot_name_log} Không có số tim nào, bỏ qua.\n")
                            break

                        max_num = max(heart_numbers)
                        if max_num < current_heart_threshold:
                            print(f"{bot_name_log} Số tim lớn nhất {max_num} < {current_heart_threshold}, không grab!\n")
                        else:
                            max_index = heart_numbers.index(max_num)
                            emoji = ["1️⃣", "2️⃣", "3️⃣"][max_index]
                            
                            # Áp dụng delay khác nhau cho mỗi bot
                            if is_bot_1:
                                delay = {"1️⃣": 0.5, "2️⃣": 1.5, "3️⃣": 2.2}[emoji]
                            else: # Bot 2
                                delay = {"1️⃣": 0.8, "2️⃣": 1.8, "3️⃣": 2.5}[emoji]

                            print(f"{bot_name_log} Chọn dòng {max_index+1} với số tim {max_num} → Emoji {emoji} sau {delay}s\n")

                            def grab():
                                try:
                                    bot.addReaction(main_channel_id, last_drop_msg_id, emoji)
                                    print(f"{bot_name_log} Đã thả emoji grab!")
                                    bot.sendMessage(ktb_channel_id, "kt b")
                                    print(f"{bot_name_log} Đã nhắn 'kt b'!")
                                except Exception as e:
                                    print(f"{bot_name_log} Lỗi khi grab hoặc nhắn kt b: {e}")
