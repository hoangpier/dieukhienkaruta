...
def auto_reboot_loop():
    """Vòng lặp chạy nền để reboot các bot chính theo định kỳ."""
...
def index():
    ...
    if request.method == "POST":
        ...
        reboot_delay_form = request.form.get("auto_reboot_delay")

        if auto_reboot_toggle:
        ...
        if reboot_delay_form:
            try:
                delay_val = int(reboot_delay_form)
                if delay_val >= 60:
                ...
                msg_status = "Lỗi: Delay không hợp lệ."
        ...
        if toggle: auto_grab_enabled = toggle == "on"; msg_status = f"Tự grab Acc chính 1 {'đã bật' if auto_grab_enabled else 'đã tắt'}"
        if toggle_2: auto_grab_enabled_2 = toggle_2 == "on"; msg_status = f"Tự grab Acc chính 2 {'đã bật' if auto_grab_enabled_2 else 'đã tắt'}"
        if toggle_3: auto_grab_enabled_3 = toggle_3 == "on"; msg_status = f"Tự grab Acc chính 3 {'đã bật' if auto_grab_enabled_3 else 'đã tắt'}"
        
        if heart_threshold_form:
            try: heart_threshold = int(heart_threshold_form); msg_status = f"Đã cập nhật mức tim Acc 1: {heart_threshold}"
            except: msg_status = "Mức tim Acc 1 không hợp lệ!"
        if heart_threshold_2_form:
            try: heart_threshold_2 = int(heart_threshold_2_form); msg_status = f"Đã cập nhật mức tim Acc 2: {heart_threshold_2}"
            except: msg_status = "Mức tim Acc 2 không hợp lệ!"
        if heart_threshold_3_form:
            try: heart_threshold_3 = int(heart_threshold_3_form); msg_status = f"Đã cập nhật mức tim Acc 3: {heart_threshold_3}"
            except: msg_status = "Mức tim Acc 3 không hợp lệ!"

        if reboot_target: reboot_bot(reboot_target); msg_status = f"Đã gửi yêu cầu reboot cho {reboot_target}!"
        if auto_work_toggle: auto_work_enabled = auto_work_toggle == "on"; msg_status = f"Auto Work {'đã bật' if auto_work_enabled else 'đã tắt'}"
    ...
    return render_template_string(HTML.format(
        ...
        auto_grab_status=auto_grab_status, auto_grab_text=auto_grab_text,
        auto_grab_status_2=auto_grab_status_2, auto_grab_text_2=auto_grab_text_2,
        auto_grab_status_3=auto_grab_status_3, auto_grab_text_3=auto_grab_text_3,
        auto_work_status=auto_work_status, auto_work_text=auto_work_text,
        ...
        auto_reboot_status=auto_reboot_status, auto_reboot_text=auto_reboot_text, auto_reboot_delay=auto_reboot_delay
    ))
